 <?php
    ...
    $sql = "SELECT `title`,`content`,`price`,`img` FROM  `electronics`  WHERE `id`=$id";
    $res = mysqli_query($kap, $sql);
    $edesc = mysqli_fetch_all($res,MYSQLI_ASSOC);
?>
<?php 
foreach($edesc as $d){
	$title = $d["title"];
 ...
 <?php
$sql =  "SELECT `id`,`title`,`img`,`price`,`content` FROM `electronics` ORDER BY `id` LIMIT 3,3";
$res = mysqli_query($kap, $sql);
$electronic = mysqli_fetch_all($res,MYSQLI_ASSOC);
?>
<article>
    <div class="container">
        <div class="row">
